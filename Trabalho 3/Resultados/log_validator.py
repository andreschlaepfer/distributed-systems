def validate():
    ...
    for line in lines:
        ...
        if ("[S] Grant" in line):
            if (len(grants) != len(releases)):
                ...
                print(len(releases))
                raise Exception(
                    "11111 Invalid log file: invalid grants and releases sequence")
            grants.append(int(line.split("-")[1]))
            continue
        if ("[R] Release" in line):
            if (len(releases) != len(grants) - 1):
                raise Exception(
                    "22222 Invalid log file: invalid grants and releases sequence")
    ...
    for i in range(len(requests)):
        if (requests[i] != grants[i] or grants[i] != releases[i]):
            raise Exception(
                "33333 Invalid log file: invalid grants and releases sequence")
